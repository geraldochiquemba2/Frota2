...
router.get("/summary", requireAuth, async (req, res) => {
  ...
  records.forEach(r => {
    ...
    monthlyMap[month].expenses += r.amount;
  });
  const monthlyData = Object.entries(monthlyMap).sort().slice(-6).map(([month, data]) => ({ month, ...data }));
  
  res.json({ totalExpenses, expensesByCategory, monthlyData });
});
...
router.post("/", requireAuth, async (req, res) => {
  const { category, description, amount, date, vehicleId, notes } = req.body;
  const [r] = await db.insert(financeRecordsTable).values({
    category, description, amount, date: new Date(date),
